...
unsigned long findXMPSignature(std::ifstream &stream, const unsigned long bufsize = 128) {
  
  // Allocate 3 buffers
  // There are 3 buffers used to prevent the rare edge case of the signature being
  // only halfway in the buffer because it is placed at for example '0x126' causing it
  // to be not completely inside
  // The first two buffers get read to and the searchbuffer is the one witht the two combined
  //
  // Example:
  ...
  // After the second read the whole word 'World' is in the buffer and can be recognized
  // It is done with 3 buffers instead of one to make the code less complex
  //
  // IMPORTANT: If the buffersize is smaller than the length of the signature it can still fail
  ...
  while (!stream.eof()) {
    ...
    if(pos != (long) std::string::npos) {
      ...
      // Twice the bufsize is used because the searchbuffer is twice as large as the other ones
